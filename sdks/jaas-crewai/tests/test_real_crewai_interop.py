...
half -- that its output is genuinely usable by real crewai.tools.BaseTool
machinery, and that a real JaasRegistryClient (against a real localhost
server, see tests/_live_server.py's docstring for why a real socket is used
instead of extracting FastAPI TestClient's internal transport) is a valid
_JaasClientLike, not just structurally similar to one.
"""
...
def test_build_jaas_tools_output_is_accepted_by_a_real_crewai_agent(app_and_client):
    tools = build_jaas_tools(app_and_client)

    agent = Agent(
        role="Skill librarian",
        goal="Find and load skills from the JaaS registry",
        backstory="You help other agents discover reusable skills.",
        tools=tools,
    )

    assert {t.name for t in agent.tools} == {"search_skills", "get_skill"}
...
def test_get_skill_tool_returns_the_real_skill_md_content(app_and_client):
    ...
    result = tools["get_skill"].run(skill_id="acme.text.summarizer", version="1.2.3")
