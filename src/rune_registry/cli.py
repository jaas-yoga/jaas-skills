"""runectl: CLI entrypoint for validating, publishing, and serving skills."""
...
if TYPE_CHECKING:
    from rune_registry.guardrails.client import GuardrailsClient


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="runectl")
...
def cmd_validate(
    ...
    from rune_registry.artifact.publish import load_source_documents
    from rune_registry.common.config import load_settings
    from rune_registry.guardrails.client import HttpGuardrailsClient
    from rune_registry.guardrails.custom_rules import CustomGuardrailRuleStore
    from rune_registry.guardrails.policy import default_policy
    ...
    # design.md §4.5: the standalone rune-guardrails service, reached only
...
def cmd_publish(
    ...
    from rune_registry.artifact.publish import load_source_documents, publish_skill
    ...
    from rune_registry.guardrails.client import HttpGuardrailsClient
    from rune_registry.guardrails.custom_rules import CustomGuardrailRuleStore
    from rune_registry.guardrails.policy import GuardrailPolicy, default_policy
    from rune_registry.guardrails.skill_config import (
        parse_skill_guardrail_config,
        read_skill_guardrail_config,
        resolve_guardrails_for_skill,
    ...
    try:
        ...
        # this skill's own .rune/guardrails.yaml before the real publish.
        ...
        with tracer.start_as_current_span("runectl.publish"):
...
def cmd_release(args: argparse.Namespace) -> int:
    ...
    if args.oidc_token:
        headers["X-Rune-OIDC-Token"] = args.oidc_token
    elif args.token:
...
def cmd_serve(args: argparse.Namespace) -> int:
    ...
    from rune_registry.observability.logging import configure_logging
    from rune_registry.observability.tracing import build_tracer
    from rune_registry.storage.local_filesystem import LocalFilesystemStore
