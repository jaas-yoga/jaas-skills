...
from rune_registry.artifact.signing import DevKeypair, sign_digest
from rune_registry.artifact.trust import TrustPolicy
from rune_registry.artifact.verify import verify_artifact
from rune_registry.common.audit import AuditSink, PublishAuditEvent, new_publish_event
from rune_registry.common.errors import ErrorCode, RuneError
from rune_registry.guardrails.certification import GuardrailCertification, compute_certification
from rune_registry.guardrails.client import CustomRuleInput, GuardrailsClient
from rune_registry.guardrails.policy import GuardrailPolicy, default_policy
from rune_registry.index.events import EventBus, new_index_update_event
...
from rune_registry.storage.keys import blob_key as make_blob_key
from rune_registry.storage.keys import tag_key as make_tag_key
from rune_registry.validation.graph import validate_no_cycles
from rune_registry.validation.models import ManifestDocument
...
def load_source_documents(source_dir: Path) -> tuple[dict[str, bytes], dict, dict, list, list]:
    ...
    source directory. Shared with runectl's `validate` command."""
...
def publish_skill(
    *,
    ...
    try:
        ...
        # rune-guardrails service over HTTP. Runs before any archive/store
    ...
    record = serialize_published_record(
        ...
        # explicitly from the caller's session. runectl publish has no
        ...
        # preserves runectl's pre-existing no-auth-needed behavior exactly.
