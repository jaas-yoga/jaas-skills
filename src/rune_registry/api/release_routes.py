...
- `X-Rune-OIDC-Token`: a GitHub Actions OIDC ID token (see
...
`artifact/publish.py::publish_skill()` — the same one `runectl publish`
...
from rune_registry.api.schemas import ReleaseRequest, ReleaseResponse
from rune_registry.artifact.packaging import build_normalized_archive, compute_digest
...
from rune_registry.authn.ci_credentials import GitHubOidcVerifier, resolve_release_tenant
...
from rune_registry.guardrails.certification import flatten_certification
...
from rune_registry.sharing.access import resolve_caller_context
...
def release_skill(
    ...
    oidc_token: Annotated[str | None, Header(alias="X-Rune-OIDC-Token")] = None,
    ...
    if manifest_bytes is None:
        raise RuneError(ErrorCode.MISSING_REQUIRED_FILE, "release is missing manifest.yaml")
    ...
    if not skill_id:
        raise RuneError(ErrorCode.MISSING_REQUIRED_FILE, "manifest.yaml is missing an id")
