...
from rune_registry.authn.github_oauth import build_authorize_url, sign_state, verify_state
...
from rune_registry.common.audit import StructuredLogAuditSink, new_github_connection_event
from rune_registry.common.config import Settings
from rune_registry.common.errors import ErrorCode, RuneError
from rune_registry.sharing.access import CallerContext, resolve_caller_context
...
def _require_membership(memberships: MembershipStore, tenant_id: str, caller: CallerContext):
    ...
    if membership is None:
        raise RuneError(ErrorCode.TENANT_NOT_FOUND, f"tenant '{tenant_id}' not found")
    return membership
...
def _require_admin(memberships: MembershipStore, tenant_id: str, caller: CallerContext):
    ...
    if membership.role is not TenantRole.ADMIN:
        raise RuneError(ErrorCode.UNAUTHORIZED, "only a tenant admin may manage its membership")
    return membership
...
def delete_github_connection(
    ...
    if not found:
        raise RuneError(ErrorCode.GITHUB_NOT_CONNECTED, f"tenant '{tenant_id}' has no connection")
