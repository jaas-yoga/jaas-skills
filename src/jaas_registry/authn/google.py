...
class RealGoogleIdentityVerifier:
    ...
    def verify(self, google_id_token_jwt: str) -> GoogleIdentity:
        ...
        if payload.get("iss") not in _VALID_ISSUERS:
            raise JaasError(ErrorCode.INVALID_GOOGLE_TOKEN, "unexpected token issuer")

        sub = payload.get("sub")
        email = payload.get("email")
        if not sub or not email:
            raise JaasError(ErrorCode.INVALID_GOOGLE_TOKEN, "token is missing sub/email claims")
        if not payload.get("email_verified", False):
            raise JaasError(ErrorCode.INVALID_GOOGLE_TOKEN, "Google account email is not verified")
