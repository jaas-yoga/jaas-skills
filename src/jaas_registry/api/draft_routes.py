...
from jaas_registry.drafts.models import Draft
from jaas_registry.drafts.store import DraftStore
from jaas_registry.guardrails.certification import compute_certification, flatten_certification
...
from jaas_registry.sharing.access import CallerContext, can_view, resolve_caller_context
...
def _require_owned_draft(drafts: DraftStore, draft_id: str, caller: CallerContext) -> Draft:
    ...
    if draft is None or draft.owner_user != caller.user_id:
        raise JaasError(ErrorCode.DRAFT_NOT_FOUND, f"draft '{draft_id}' not found")
    return draft
...
def _connect_draft_to_git(
    ...
    *,
    ...
    if connection is None:
        drafts.delete(draft.id)
        raise JaasError(ErrorCode.GITHUB_NOT_CONNECTED, "this tenant has not connected GitHub")
    ...
    try:
        working_branch = (
            request_git.workingBranch or f"jaas/draft/{draft.id.removeprefix('draft_')}"
        )
...
def get_draft_file(
    ...
    if content is None:
        raise JaasError(ErrorCode.DRAFT_NOT_FOUND, f"file '{file_path}' not found in draft")
...
def move_draft_to_git_directory(
    ...
    if draft.repo_url is None:
        raise JaasError(ErrorCode.DRAFT_GIT_NOT_CONNECTED, "this draft is not connected to a repo")
    if draft.git_subdirectory:
        raise JaasError(
...
def _load_draft_documents(drafts: DraftStore, draft_id: str):
    ...
    letting a JaasError from that path surface is correct — /validate's own
    `except JaasError` branch already reports it cleanly."""
    manifest_bytes = drafts.read_file(draft_id, "manifest.yaml")
    if manifest_bytes is None:
...
def publish_draft(
    ...
    # skill's own .jaas/guardrails.yaml before that call.
