...
A PAT lets `jaasctl` (no browser, can't run the OAuth dance) authenticate
...
from jaas_registry.api.deps import AuthorizerDep, PatStoreDep, SettingsDep, UserStoreDep
from jaas_registry.api.schemas import CreatePatRequest, CreatePatResponse, PatSummaryResponse
from jaas_registry.authn.models import TenantRole
from jaas_registry.authn.tokens import mint_access_token
...
def create_pat(
    ...
    if ttl_seconds <= 0:
        raise JaasError(ErrorCode.SCHEMA_VALIDATION_FAILED, "ttlSeconds must be positive")
...
def revoke_pat(
    ...
    if not pat_store.revoke(pat_id=pat_id, owner_user=claims.subject):
        raise JaasError(ErrorCode.PAT_NOT_FOUND, f"personal access token '{pat_id}' not found")
