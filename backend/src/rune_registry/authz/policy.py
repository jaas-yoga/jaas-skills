...
missing scope, tenant mismatch) raises RuneError(UNAUTHORIZED) — there is no
...
from rune_registry.authz.scopes import has_all_required_scopes
from rune_registry.common.config import Settings
from rune_registry.common.errors import ErrorCode, RuneError
from rune_registry.observability.metrics import authz_denied_total
...
class JwtAuthorizer:
    ...
    def _check(
        ...
        if not token:
            raise RuneError(ErrorCode.UNAUTHORIZED, "missing bearer token")
        ...
        if claims.pat_id is not None:
            ...
            if record is None:
                raise RuneError(ErrorCode.UNAUTHORIZED, "personal access token has been revoked")
