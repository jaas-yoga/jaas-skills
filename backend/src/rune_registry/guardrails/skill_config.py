"""Parses a skill package's optional `.rune/guardrails.yaml` — lets a
...
from rune_registry.guardrails.custom_rules import CustomGuardrailRule, CustomGuardrailRuleStore
from rune_registry.guardrails.custom_rules import validate_slug as _validate_slug
from rune_registry.guardrails.policy import GuardrailPolicy

MAX_INLINE_RULES = 20
_REQUIRED_INLINE_KEYS = {"slug", "name", "category", "severity", "kind", "config"}

GUARDRAILS_CONFIG_PATH = ".rune/guardrails.yaml"
...
def parse_skill_guardrail_config(raw: bytes | str | None) -> SkillGuardrailConfig:
    ...
    if not isinstance(raw_rules, list):
        raise RuneError(ErrorCode.INVALID_CUSTOM_GUARDRAIL, "'rules' must be a list")
...
def resolve_guardrails_for_skill(
    *,
    ...
    skill's own `.rune/guardrails.yaml` additionally applies. Returns
