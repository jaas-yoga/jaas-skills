---
name: rune-backend-conventions
description: Conventions, known gotchas, and repo/git state for the rune-registry Python/FastAPI backend (src/rune_registry) — error codes, the file-backed no-database persistence pattern, FastAPI dependency wiring, the visibility/sharing model, test fixtures, and this repo's own commit/CI/push conventions. Use when reading, writing, reviewing, or committing any code under src/rune_registry or tests/ in this repo.
---

# rune-registry backend conventions
...
sibling `rune_ui` repo, since they also cover the backend changes built to
...
independent sibling repo **`rune_ui`**, reached only over HTTP
(`RUNE_API_URL`); the publish-time guardrails scanning engine lives in the
independent sibling repo/service `rune-guardrails-catalog` (see the
...
- One `ErrorCode` StrEnum, one `HTTP_STATUS_BY_CODE` dict, one `RuneError`
  exception (`code`, `message`, `details`). Every rejection anywhere in the
  codebase raises `RuneError`, never a bare exception, so the API's global
...
- **`app.state.trust_policy` is a fixed snapshot from `create_app()` time.**
  ...
  `runectl publish`) must call `load_trust_policy(settings.policy_dir)`
...
- **This repo contains no scanning logic and no rule catalog.** Both live
  entirely in the separate [rune-guardrails-catalog](https://github.com/balakrishna-maduru/rune-guardrails-catalog)
...
- `guardrails/models.py` is a **hand-kept mirror** of that service's
  response shapes (same pattern as the sibling `rune_ui` repo's
  `src/lib/rune-api-types.ts` mirroring this app's own schemas) —
  `GuardrailDefinition`,
...
- `guardrails/skill_config.py` parses a skill package's optional
  `.rune/guardrails.yaml` (`apply:` a list of catalog or tenant custom
...
- `artifact/publish.py`'s `publish_skill()` takes `guardrails_client:
  ...
  `RuneError(GUARDRAIL_VIOLATION)` before any archive/store write; WARN
...
- `POST /api/v1/skills/release` is a **third** front door to
  `publish_skill()`, alongside the web UI's draft-publish and `runectl
...
- Two mutually exclusive auth paths, resolved in `_resolve_tenant()`:
  `X-Rune-OIDC-Token` (GitHub Actions OIDC, verified by
...
- CLI side: `runectl release` (`cli.py::cmd_release`) is a pure HTTP
  client of this endpoint — it's the *only* `runectl` command that talks
  ...
  `storage_root`/the signing key. `runectl guardrails push` similarly
  calls the custom-guardrails CRUD API; `runectl guardrails validate`
...
- **`.github/workflows/ci.yml` only covers this repo** — checkout, `uv
  ...
  awareness of the sibling `rune_ui`/`rune_guardrail` repos; each of those
